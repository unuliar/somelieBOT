...
def callback_worker(call):
    if call.data == "red":
        ...
        get_wine_from_api(call.message, config.static_params + dbworker.get_current_state(str(call.message.chat.id) + '_data').decode())
        return 0
...
def cmd_start(message):
    state = dbworker.get_current_state(message.chat.id).decode()
    print(state)
...
@bot.message_handler(func=lambda message: dbworker.get_current_state(message.chat.id).decode() == config.States.S_ENTER_WINE_PRICE.value)
...
@bot.message_handler(func=lambda message: dbworker.get_current_state(message.chat.id).decode() == config.States.S_ENTER_WINE_RATING.value)
def user_entering_rating(message):
    dbworker.add_to_end(str(message.chat.id) + '_data', '&min_rating=' + message.text)
    get_wine_from_api(message, config.static_params + dbworker.get_current_state(str(message.chat.id) + '_data').decode())
...
def get_wine_from_api(message, params):
    ...
    if viewed >= len(wines):
        bot.send_message(message.chat.id, 'Вино кончилось(( /reset')
    wine = wines[viewed]
